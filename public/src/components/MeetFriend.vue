      ...
      <div class="row">
        ...
        <div class="col-12 col-sm-4" v-if="loading == false">
          <div>
            ...
            <div v-if="show == true">
              <button class="btn btn-sm marg-bot orange btn-block" @click="show = false, initMap()">New Locations</button>
              <div v-if="geoShow == false">
                <h4>Your Location: </h4>
              ...
              <form @submit.prevent="submitPlace()">
                ...
                <button class="btn teal btn-block" type="submit">Search</button>
              </form>
              <div class="list-group marg-top">
                <div v-if="this.placesResults == false">
                  <h5>No Results Found...</h5>
                  <h6>Try adjusting the radius or click and drag the mid-point.</h6>
                </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      // TAKES IN THE USER DATA AND SETS OFF THE FUNCTIONS TO STORE ORIGIN AND DESTINATION
      getTrip() {
        this.show = true
      ...
      // CREATES MAP WITH ORGIN, DESTINATION, AND MID-POINTS
      setMap(origin, destination) {
        ...
        // CREATE BOUNDS OF THE ZOOM
        var bounds = new google.maps.LatLngBounds()
        bounds.extend(start);
        bounds.extend(end)
        // SET MAP TO id="map" FROM ABOVE DIV
        const element = document.getElementById('map')
        const options = {
        ...
        // USE EITHER GEOLOCATION OR THE USER INPUT
      ...
      // FINDS THE DRIVING MID-POINT
      findDrivingMidPoint(start, end) {
        var scope = this
        // SETS IT UP AS A DRIVING SERVICE
        ...
        // TAKES IN THE START AND END AND CALCULATES THE MID-POINT OF THOSE TWO LOCATIONS
      ...
      // FIRES OFF TO GOOGLE API TO GET THE PALCES AROUND THAT MID-POINT LOCATION
      submitPlace() {
        this.$store.dispatch('getPlaces', {
          midway: this.roadMidway,
          category: this.type,
          radius: this.radius,
          length: this.totalResults
        })
        this.addCircle({
          location: this.roadMidway,
          radius: this.radius
        })
      ...
      geoShowToggle() {
        this.trip.geolocation = {};
        this.geoShow = false
        var string = this.trip.destination.split("+").join(" ");
...
<style scoped>
  ...
  .orange:hover {
      background-color: rgb(204, 105, 78)
  }
